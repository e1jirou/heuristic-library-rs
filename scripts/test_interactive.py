...
#       ├── in/ # input
#       ├── out/ # output
#       └── target/release/tester # local tester
...
TOOL_DIR = "tools"
TOOL_COMPILE_CMD = "cargo build --release --bin tester"

EXEC_CMD = "./tools/target/release/tester ./target/release/ahc"
...
def main():
    ...
    max_time = np.max(times) if len(times) <= NUM_PARALLELS else np.max(times[NUM_PARALLELS:])
    ...
    print(f"Time: Avg. {np.mean(times):.3f}, Min. {np.min(times):.3f}, Max. {max_time:.3f}")
